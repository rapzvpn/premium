...
wget -q -O /root/backup/backup.zip "https://raw.githubusercontent.com/rapzvpn/backup/master/$nama/$nama.zip" &> /dev/null
...
echo -e "$COLOR1│${NC}  [ ${green}INFO${NC} ] • Done... Successfully - Script BY rapzvpn PREMIUM"
sleep 1
rm -f /root/backup/backup.zip &> /dev/null
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}" 
echo ""
